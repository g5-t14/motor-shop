...
export const Product = () => {
  ...
  useEffect(()=>{
    (async () => {
      ...
      const picturesFiltered = Object.values(carPictures).filter(element => element != null)
  ...
  const formatAdTitle = (brand: string, name: string):string => {
    ...
    const formatedName = name.charAt(0).toUpperCase() + name.split(" ")[0].slice(1);
    ...
    <main className="py-[45px] px-3 flex flex-wrap max-w-[1240px] mx-auto justify-center midson:justify-between">
      {
        ...
        <div className={"md:max-w-[750px] md:h-[355px] max-h-[355px] w-full h-[355px] rounded flex items-center bg-grey10 mb-[17px] bg-cover bg-center bg-[url('" + car?.cover_img + "')]"} >
        </div>

        <section className="rounded flex flex-col gap-8 px-7 py-9 bg-grey10 mb-[15px] w-full">
          <h2 className="text-[20px] leading-[25px] font-semibold font-lexend text-grey1">{car ? formatAdTitle(car.brand,car.model): "Title not found!"}</h2>
          <div className="flex gap-3">
            <span className="pt-1 pb-1 pl-2 pr-2 bg-brand4 text-brand1 rounded font-medium text-[14px] leading-6">{car.mileage }KM</span>
            <span className="pt-1 pb-1 pl-2 pr-2 bg-brand4 text-brand1 rounded font-medium text-[14px] leading-6">{car.year}</span>
          </div>
          <span className="font-lexend text-[16px] leading-5 font-medium">{(car.price).toLocaleString('pt-br',{style: 'currency', currency: 'BRL'})}</span>
        ...
        <section className="rounded flex flex-col gap-8 px-7 py-9 bg-grey10 mb-[15px] w-full">
          ...
          <p className="text-grey2 font-normal text-[16px] leading-[28px] min-w-[100%] w-full">{car.description}</p>
        </section>
      </div>
        ...
        <section className="rounded flex flex-col gap-8 px-7 py-9 bg-grey10 mb-[52px]">
          ...
          <ul className="flex flex-wrap justify-evenly gap-3">
              {carPictures.map((element,index) => {
                return <li key={index} className="w-[90px] max-w-[100%] h-[90px] rounded">
                <img className="w-full h-full rounded" src={element} alt="" />
              </li>
              })}
          </ul>
        </section>
        <section className="flex flex-col gap-7 px-7 py-10 items-center rounded bg-grey10 mb-[18px]">
          <div className={`bg-[${car.user_seller.user_color}] rounded-full w-[77px] h-[77px] flex items-center justify-center`}>
            <span className="text-white font-medium text-[27px] leading-[40px]">{car.user_seller.name.charAt(0).toUpperCase()}</span>
          </div>
          <h2 className="text-[20px] leading-[25px] font-semibold font-lexend text-grey1">{car.user_seller.name}</h2>
          ...
          <Link className="bg-grey0 border-grey0 hover:bg-grey1 hover:border-grey1 btn-big flex items-center" to={`/advertiser/${car.user_seller.id}`}>Ver todos os anúncios</Link>
        </section>
      </div>
