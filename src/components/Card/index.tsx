...
export const Card = ({brand,description,fipe_table,fuel,id,is_active,mileage,name,value,year,seller}:CardProps) => {
  ...
  return(
    <li className="min-w-[312px] min-h-[350px] w-[312px] h-[350px] flex flex-col gap-4 mb-[85px]"> 
      ...
      <div className="flex flex-col gap-4">
        <h3 className="truncate font-semibold text-[16px] leading-5 text-grey1">{formatAdTitle(brand,name)}</h3>
        <p className="text-grey2 font-normal text-[14px] leading-6 truncate whitespace-normal line-clamp-2">{description}</p>
        <div className="flex items-center gap-2 ">
          <div className="rounded-full bg-purple-600 w-8 h-8 flex items-center justify-center">
            <span className="text-white font-medium text-[14px]">{seller.charAt(0).toUpperCase()}</span>
          </div>
